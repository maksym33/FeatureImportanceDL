...
class MaskOptimizer():
    def __init__(self,mask_batch_size,data_shape,unmasked_data_size, frac_of_rand_masks=0.5,epoch_condition =1000,record_best_masks=False, maximize_error=False,perturbation_size = 1,use_new_optimization=False):
        ...
        self.record_best_masks = record_best_masks
        self.maximize_error = maximize_error
        self.perturbation_size = perturbation_size
        self.use_new_mask_optimization = use_new_optimization
        self.max_optimization_iters = 5
        self.step_count_history = []
    def maximize_output_for_input(model,iters,lr,eps=0.0000001):
        ...
        for i in range(iters):
            grads,loss = gradient(model, input_img)
            norm = np.linalg.norm(grads[0]+eps)
            grads = grads[0]/norm
            input_img += grads * lr  # update the image based on gradients
            input_img = np.where(input_img>1,1,input_img)
            input_img = np.where(input_img<0,0,input_img)
            losses.append(loss)
        #print(input_img)
        return input_img,np.array(losses) 
    def minimize_output_for_input(model,iters,lr):
        #input_img = np.random.uniform(0,1,mask_size)[None,:] # define an initial random image        
        input_img = np.ones(shape = model.layers[0].output_shape[0][1:])[None,:]/2 # define an initial random image
        losses=[]
        for i in range(iters):
            grads,loss = MaskOptimizer.gradient(model, input_img)
            norm = np.linalg.norm(grads[0])
            grads = grads[0]/norm
            input_img -= grads * lr  # update the image based on gradients
            input_img = np.where(input_img>1,1,input_img)
            input_img = np.where(input_img<0,0,input_img)
    ...
    def new_get_m_opt(model,unmasked_size):
        ...
        #print(grad)
        #print(np.argsort(grad))
        #top_arg_grad = np.argpartition(grad, -unmasked_size)[-unmasked_size:]
        #print(top_arg_grad)
        #m_opt[top_arg_grad] = 1
        return m_opt
    ...
    #def new_get_min_opt_grad(m_opt_grad,m_opt_indexes):
        #print(m_opt_grad)
        #print(m_opt_indexes)
        #m_opt_grad_cp = np.copy()     
        #return m_opt_indexes[np.argmin(m_opt_grad[m_opt_indexes])]
    ...
    def new_check_for_likely_change(model,m_opt,min_index,max_index,m_opt_grad):
        ...
        if(m_new_opt_loss < m_opt_loss):
            ...
            return False,m_opt        
    def get_opt_mask(self,unmasked_size,model,iters,lr,steps=None):
        if(self.use_new_mask_optimization==True):
            ...
            while(repeat_optimization==True and step_count<steps):
                ...
                repeat_optimization, m_opt = MaskOptimizer.new_check_loss_for_opposite_indexes(model,m_opt,min_index,max_index,opposite_indexes)
                if(repeat_optimization==True):
                    #print("Repeating due negative indexes for unmasked inputs")
                    continue
                repeat_optimization, m_opt = MaskOptimizer.new_check_for_likely_change(model,m_opt,min_index,max_index,m_opt_grad)
                if(repeat_optimization==True):
            ...
            if(self.maximize_error == False):
                raw_mask,model_loss = MaskOptimizer.minimize_output_for_input(model,iters,lr)
            else:
                raw_mask,model_loss = MaskOptimizer.maximize_output_for_input(model,iters,lr)
            self.loss_history.append(model_loss)
            self.raw_mask_history.append(raw_mask)
            mask = np.zeros(shape = raw_mask.shape)
            best_indices = np.argsort(raw_mask,axis=1)[0,-unmasked_size:]
            #print(best_indices)
            mask[:,best_indices] = 1
    ...
    def perturb_masks(masks):
        ...
        return masks
    def get_new_mask_batch(self,model,best_performing_mask,iters,lr,gen_new_opt_mask):
        self.epoch_counter += 1
        random_masks = self.get_random_masks()
        if(gen_new_opt_mask):
                self.mask_opt = self.get_opt_mask(self.unmasked_data_size,model,iters,lr)
                #print("Opt: "+str(np.squeeze(np.argwhere(self.mask_opt==1))))
                #print("Perf: "+str(np.squeeze(np.argwhere(best_performing_mask==1))))
        if(self.check_condiditon()==True):
            ...
            random_masks[index+2:]= MaskOptimizer.get_perturbed_masks(random_masks[index], self.mask_batch_size - (index+2),self.perturbation_size)
        ...
        elif(self.record_best_masks==True):
            self.get_opt_mask(self.unmasked_data_size,model,iters,lr)
        return random_masks
